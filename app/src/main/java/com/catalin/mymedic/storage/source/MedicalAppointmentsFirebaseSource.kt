...
//        }
    ...
    fun getFutureMedicalAppointments(userId: String, timestamp: Long): Flowable<List<MedicalAppointment>> =
        RxFirebaseDatabase.observeValueEvent(firebaseDatabase.reference.child(FirebaseDatabaseConfig.MEDICAL_APPOINTMENTS_TABLE_NAME), { snapshot ->
            val result = ArrayList<MedicalAppointment>()
            val appointments = snapshot.children.mapNotNull { value -> value.getValue(MedicalAppointment::class.java) }.sortedBy { it.dateTime }
            val first = appointments.filter { it.medicId == userId && it.dateTime >= timestamp && it.status == AppointmentStatus.CONFIRMED }
            if (first.isNotEmpty()) {
                result.add(MedicalAppointment().apply { id = Constants.PATIENT_APPOINTMENTS_HEADER_ID })
                result.addAll(first)
            }
            val second =
                appointments.filter { it.patientId == userId && it.dateTime >= timestamp && (it.status == AppointmentStatus.AWAITING || it.status == AppointmentStatus.CONFIRMED) }
                    .sortedBy { it.dateTime }
    ...
    fun getPastAppointments(userId: String, timestamp: Long): Flowable<List<MedicalAppointment>> =
        RxFirebaseDatabase.observeValueEvent(firebaseDatabase.reference.child(FirebaseDatabaseConfig.MEDICAL_APPOINTMENTS_TABLE_NAME), { snapshot ->
            val result = ArrayList<MedicalAppointment>()
            val appointments = snapshot.children.mapNotNull { value -> value.getValue(MedicalAppointment::class.java) }.sortedBy { it.dateTime }
            val first =
                appointments.filter { it.medicId == userId && ((it.dateTime <= timestamp) || (it.status == AppointmentStatus.REJECTED || it.status == AppointmentStatus.CANCELED_BY_MEDIC || it.status == AppointmentStatus.CANCELED_BY_PATIENT)) }
            if (first.isNotEmpty()) {
                result.add(MedicalAppointment().apply { id = Constants.PATIENT_APPOINTMENTS_HEADER_ID })
                result.addAll(first)
            }
            val second =
                appointments.filter { it.patientId == userId && ((it.dateTime <= timestamp) || (it.status == AppointmentStatus.REJECTED || it.status == AppointmentStatus.CANCELED_BY_MEDIC || it.status == AppointmentStatus.CANCELED_BY_PATIENT)) }
                    .sortedBy { it.dateTime }
            if (second.isNotEmpty()) {
                result.add(MedicalAppointment().apply { id = Constants.OWN_APPOINTMENTS_HEADER_ID })
                result.addAll(second)
...
//        ).equalTo(userId), { data ->
//            data.children.mapNotNull { value -> value.getValue(MedicalAppointment::class.java) }
//                .filter { (it.dateTime <= timestamp) || (it.status == AppointmentStatus.REJECTED || it.status == AppointmentStatus.CANCELED_BY_MEDIC || it.status == AppointmentStatus.CANCELED_BY_PATIENT) }
//                .sortedBy { it.dateTime }
//        }).flatMap { firstResult ->
//            val result = ArrayList<MedicalAppointment>().apply {
//                if (firstResult.isNotEmpty()) {
